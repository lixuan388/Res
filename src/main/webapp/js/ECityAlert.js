...
var AlertConfirm =
  "<div class=\"modal fade\" id=\"AlertConfirm\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"myModalLabel\">\r\n"
...
//
//  var AlertConfirm =
//    $("<div class=\"modal \"  tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"myModalLabel\" style=\"background-color: #ccccccb3;z-index: 6000;\">\r\n"
...
function OpenWindows(href, title, onClose, width, height) {
  ...
  var OpenModal =
    $("	<div id=\"ModalID" + modalID + "\" class=\"modal\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"myModalLabel\" style=\"background-color: #ccccccb3;\"> \r\n"
      ...
      + "</h4>\r\n" + "				</div>\r\n" + "				<div class=\"modal-body\" style=\"width:100%;height:calc(100% - 60px);overflow: hidden;padding: 0px;\">\r\n" + "					<iframe  name=\"ModalID" + modalID
      + "\" src=\"" + href + "\" scrolling=\"auto\" frameborder=\"0\" width=\"100%\" height=\"100%\" ></iframe>\r\n" + "				</div>\r\n" + "			</div> \r\n"
      + "<div class=\"loadingToast\" style=\" display: none1;\">" + "<div class=\"weui-mask_transparent\"></div>" + "<div class=\"weui-toast\">" + "<i class=\"weui-loading weui-icon_toast\"></i>"
      + "<p class=\"weui-toast__content\">数据加载中</p>" + "</div>" + "</div>" + "		</div> \r\n" + "	</div>");
...
function loadingToast(text, onClose) {
  ...
  var loading =
    $('<div id="loadingToast" style=" display: none;">' + '<div class="weui-mask_transparent"></div>' + '<div class="weui-toast">' + '<i class="weui-loading weui-icon_toast"></i>'
      + '<p class="weui-toast__content">' + content + '</p>' + '</div>' + '</div>');
  var Body = GetParentBody(window.parent);
  $(Body).append(loading);
...
function LoadingShow(text, onClose) {
  var content = text ? text : "数据加载中"
  var Body = GetParentBody(window.parent);
  //var Body =$("body", window.document);
  $(Body).append(
      elemLoad = $('<div id="loadingToast" ><div class="weui-mask_transparent"></div><div class="weui-toast"><i class="weui-loading weui-icon_toast"></i><p class="weui-toast__content">'+content+'</p></div></div>')
  );
}
